...
namespace RAFSystem
{
    public partial class UpdateRaf : Form
    {
        ...
        {
            if(Int32.Parse(totalprice.Text) > 0){ 
            ...
            {
                MessageBox.Show("Unable to Update the RAF");
        ...
        private void printraf_Click(object sender, EventArgs e)
        {
            ...
            String fileName2 = id.Text + "RAF" + ".csv";
            ...
            string[] output1 =
           {
            ...
            for (int i = 0; i < output1.Length; i++)
            {
                sw.WriteLine(output1[i].ToString());
            }
            for (int i = 0; i < items.Length; i++)
            {
                sw.WriteLine(items[i].Text.ToString());
            }
            sw.Flush();
            sw.Close();
